...
2. **Bật mạng Sepolia:**
   - Mở MetaMask → Nhấp dropdown mạng (góc trên trái, mặc định ghi "Ethereum Mainnet").
   - Bật **Show test networks** → Chọn **Sepolia**.

3. **Nhận Sepolia ETH miễn phí:**
   - Copy địa chỉ ví (nhấp vào tên tài khoản ở MetaMask để copy).
   - Truy cập faucet:
...
4. **Lấy Private Key** (cần cho deploy Smart Contract):
   - MetaMask → Biểu tượng 3 chấm → **Account details** → **Show private key** → Nhập mật khẩu MetaMask → Copy chuỗi ký tự.
